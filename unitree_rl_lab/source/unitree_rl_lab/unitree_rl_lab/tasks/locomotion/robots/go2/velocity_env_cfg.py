...
COBBLESTONE_ROAD_CFG = terrain_gen.TerrainGeneratorCfg(          #地形生成器
    size=(8.0, 8.0),
    border_width=20.0,
    num_rows=10,
    num_cols=20,
    ...
    use_cache=False,
    sub_terrains={                      #对一整块地形进行不同类型地形的划分
...
class RobotSceneCfg(InteractiveSceneCfg):
    ...
    terrain = TerrainImporterCfg(
        ...
        # 使用平地地形（关闭地形生成器）
        # terrain_type="plane",      # ← 使用平面
        # terrain_generator=None,    # ← 不使用生成器
...
class ObservationsCfg:          #策略观测组
    ...
    @configclass
    class CriticCfg(ObsGroup):           #特权信息60维
        ...
        joint_effort = ObsTerm(func=mdp.joint_effort, scale=0.01, clip=(-100, 100))                   # 对应公式(2)的哪个参数 dt
...
class RewardsCfg:
    ...
    #Orientation
    flat_orientation_l2 = RewTerm(func=mdp.flat_orientation_l2, weight=-0.2)  # 机器倾斜惩罚
    #Joint accelerations
    ...
    #smoothness - 惩罚动作的二阶差分（加加速度jerk），让动作变化更平滑
    action_smoothness = RewTerm(func=mdp.action_smoothness_l2, weight=-0.01)

    #power distribution - 惩罚各关节功率分布的方差，鼓励功率均匀分布
    ...
    # -- monitoring (weight=0, only for logging)
    foot_height_monitor = RewTerm(
        func=mdp.monitor_foot_height,
        weight=0.0,  # 不影响训练，仅用于监控
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=".*_foot"),
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_foot"),
            "print_interval": 20,  # 每20步打印一次（训练模式）
        },
    )
...
class CurriculumCfg:
    ...
    terrain_levels = CurrTerm(func=mdp.terrain_levels_vel)  # 关闭地形课程
...
class RobotPlayEnvCfg(RobotEnvCfg):
    def __post_init__(self):
        ...
        # Play模式下更频繁地打印足部高度信息
        self.rewards.foot_height_monitor.params["print_interval"] = 10  # 每10步打印一次
